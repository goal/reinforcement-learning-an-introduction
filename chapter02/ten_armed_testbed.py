...
class Bandit:
    ...
    def step(self, action):
        ...
        self.time += 1
        self.average_reward = (
            self.time - 1.0) / self.time * self.average_reward + reward / self.time
        self.action_count[action] += 1

        if self.sample_averages:
            # update estimation using sample averages
            self.q_estimation[
                action] += 1.0 / self.action_count[action] * (reward - self.q_estimation[action])
        elif self.gradient:
            ...
            if self.gradient_baseline:
                ...
                baseline = 0
            self.q_estimation = self.q_estimation + self.step_size * \
